...
class HeadlessInspectorAdapter:
    ...
    def scan_dir_generator(self, path: Path, query: str = "", max_peek_depth: int = 2, current_depth: int = 0) -> Generator[Union[FileNode, ScanStatus], None, None]:
        ...
        # For a standard scan, we only look at depth 0.
        # But for search, we use depth 1 peeking to show matching folders.
        config: ScanConfig = {
            "root_path": str(path),
            "max_depth": 0,
    ...
    # Mutation Stubs (Headless is Read-Only)
    @staticmethod
...
class ExplorerComponent(ttk.Frame):
    ...
    def _load_icons(self):
        ...
        # Resolve assets path relative to script
    ...
    def _setup_ui(self):
        ...
        # Initialize sizing: #0 is the main flexible column
        ...
        # Bind resize event to maintain column proportions
    ...
    def set_root(self, path: Path):
        ...
        # Consistent IID normalization across all platforms to prevent duplicate/invalid ID collisions
    ...
    def set_theme(self, palette: dict):
        ...
        # Ensure breadcrumbs (tk.Buttons) are recreated with new palette
        self._update_breadcrumbs()

    def _on_tree_resize(self, event):
        """Maintains readable column distribution on resize."""
        tw = event.width
        if tw < 300: return
        # Keep metadata columns stable, give remaining to name
        size_w = 100
        date_w = 150
    ...
    def populate_node(self, node, path: Path, level=0, sid=None):
        ...
        # Defensive check: node might have been deleted from tree before this call
    ...
    def _scan_worker(self, node, path: Path, level, sid, filter_text):
        # Optimization: Early check to skip processing if this scan is already stale
        with self._state_lock:
        ...
        try:
            for result in generator:
                ...
                if isinstance(result, dict):
                    ...
                    # Consistent IID normalization
                    raw_iid = str(result["path"])
                    normalized_iid = raw_iid.lower()
    ...
    def _process_queue_loop(self):
        ...
        try:
            while True:
                ...
                # Defensive check: the parent node might have been deleted from the UI
                if node and node != "" and not self.tree.exists(node): continue
                ...
                if msg_type == "items":
                    ...
                    for item in data:
                        ...
                        try:
                            if not self.tree.exists(iid):
                                ...
                                # Insert dummy child for folders if no filter
                                if item["is_dir"] and not q:
                                    try:
                                        self.tree.insert(n, "end", text="dummy")
                                    except _tkinter.TclError as te:
                                        print(f"[UI ERROR] Failed to insert dummy for {iid}: {te}", flush=True)
                        except _tkinter.TclError as te:
                            print(f"[UI ERROR] TclError inserting {iid} under {node}: {te}", flush=True)
                        except Exception as e:
                            print(f"[UI ERROR] Unexpected error inserting {iid}: {e}", flush=True)
                    ...
                    try:
                        ...
                        print(f"[UI ERROR] Status update TclError for {node}: {te}", flush=True)
                    except Exception as e:
                        print(f"[UI ERROR] Status update error for {node}: {e}", flush=True)
    ...
    def on_expand(self, event):
        ...
        if len(children) == 1 and self.tree.item(children[0], "text") == "dummy":
            ...
            p = Path(node)
            if p.exists():
                self.populate_node(node, p, level=lvl)
            else:
                print(f"[UI ERROR] Expansion failed: Path no longer exists: {node}", flush=True)
    ...
    def _internal_on_double_click(self, event):
        ...
        if p:
            if not p.exists():
                messagebox.showwarning("Unavailable", f"Path no longer exists:\n{p}")
                self.refresh()
                return
...
class InspectorApp:
    def __init__(self, root_path=None, inspector=None):
        self.root = tk.Tk(); self.root.title("File Explorer Plugin"); self.root.geometry("1100x750")
